...
<template>
    <app-layout title="Update Product">
        ...
        <v-sheet class="bg-white px-4 py-12">
            <v-form @submit.prevent="form.put(route('products.update', product.id))">
                ...
                <v-row>
                    ...
                    <v-col cols="auto">
                        <v-btn :href="route('products.show', product.id)" color="error" variant="flat" prepend-icon="mdi-close">Cancel</v-btn>
                    </v-col>
                </v-row>
            </v-form>
        </v-sheet>
